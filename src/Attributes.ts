...
export default class Attributes {
    ...
    public static set(text: string, name: string, value: string): string {
        const start = text.indexOf(name);
        if (start === -1) {
    ...
    public static toJSON(text: string, lowerCaseAttributeNames = false) {
        ...
        for (let i = 0; i < text.length; ++i) {
            ...
            if (isWhitespace(char)) {
                ...
                else if (isParsingValue) value += char;
                continue;
            }
    ...
    public static toString(attr: Record<string, string | true>): string {
        const attrs: string[] = [];
